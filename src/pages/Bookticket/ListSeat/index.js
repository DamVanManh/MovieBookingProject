...
export default function ListSeat() {
  ...
  const classes = useStyles({ color: colorTheater[thongTinPhim?.tenCumRap.slice(0, 3).toUpperCase()], modalLeftImg: thongTinPhim?.hinhAnh, isMobile, widthLabel: widthSeat / 2 })
  const dispatch = useDispatch();

  useEffect(() => { // khởi tạo event lắng nghe "resize"
  ...
  const handleSelectedSeat = (seatSelected) => {
    ...
    const newListSeatSelected = newListSeat?.reduce((newListSeatSelected, seat) => {
      if (seat.selected) {
      ...
      <div className={classes.overflowSeat}>
        <div className={classes.invariantWidth}>
          ...
          <div className={classes.seatSelect}>
            {listSeat?.map((seat, i) => (
              <div className={classes.seat} key={seat.maGhe} ref={domToSeatElement}>
                ...
                {seat.daDat && <img className={classes.seatLocked} src="/img/bookticket/notchoose.png" alt="notchoose" />}
                {/* icon ghế */}
